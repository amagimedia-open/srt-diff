...
SYNOPSIS
    ...
    $MODNAME -r rangespec_filepath [-h] [srtlev_filepath]
...
DETAILS
    ...
    The rangespec_filepath must be as follows
    BEGIN_I, END_E, NAME
    ......,  ....,  ....
    ......,  ....,  ....
    ...
    The default rangespec_filepath is as follows:
    `cat $DEF_RANGE_SPEC_FILE_PATH | sed 's/^/    /'`

    When this script is run from a docker, the strlev_filepath and
    rangespec_filepath must be anchored at the '/data' folder (i.e.
    they must have the form '/data/.../{filepath}' where '/data' is 
    the folder that is mapped through the -v option of the docker
    run command.

OPTIONS

    -r  rangespec_filepath
        this is optional.
